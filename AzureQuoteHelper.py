...
class GetAzurePricing(BaseModel):
    vm_size: str
    disk_type: str  # e.g., 'Premium SSD'
    disk_size: str  # e.g., '400GB'
    region: str

# Define the function that fetches Azure pricing
@st.cache_data
def get_azure_pricing(vm_size: str, disk_type: str, disk_size: str, region: str):
    ...
    Parameters:
    - vm_size (str): The size of the virtual machine (e.g., 'b2ms').
    - disk_type (str): The type of disk (e.g., 'Premium SSD').
    - disk_size (str): The size of the disk (e.g., '400GB').
    - region (str): The Azure region (e.g., 'US East').
    ...
    try:
        ...
        # Fetch VM pricing
        vm_filter = f"serviceName eq 'Virtual Machines' and armSkuName eq '{vm_size}' and armRegionName eq '{region}'"
        vm_response = requests.get(
            AZURE_PRICING_API_URL,
            params={"$filter": vm_filter, "$top": 1000, "api-version": "2023-01-01-preview"}
        )
        ...
        # Fetch Disk pricing
        disk_filter = f"serviceName eq 'Storage' and productName eq '{disk_type}' and armRegionName eq '{region}'"
        disk_response = requests.get(
            AZURE_PRICING_API_URL,
            params={"$filter": disk_filter, "$top": 1000, "api-version": "2023-01-01-preview"}
        )
...
def visualize_recommendations(recommendations: dict):
    ...
    vm_size = df["vm_size"].iloc[0]
    disk_size = df["disk_size"].iloc[0]
    ...
    fig = px.bar(
        ...
        labels={"x": "Cost Component", "y": f"Price ({currency})"},
        title=f"Pricing Breakdown for {vm_size} with {disk_size} in {region}"
    )
    fig.update_traces(texttemplate=f'${{y:.4f}}', textposition='outside')
...
def handle_file_upload(uploaded_file):
    ...
    with st.spinner("Analyzing quote and generating VM recommendations..."):
        # Refined Prompt: Specific instructions to extract parameters and adhere to schema
        prompt = (
            ...
            f"Provide the recommendations in a JSON format adhering strictly to the following schema:\n\n"
            f"{json.dumps(GetAzurePricing.schema(), indent=2)}\n\n"
            f"Constraints:\n"
            f"- vm_size must be a valid Azure VM size (e.g., 'b2ms').\n"
            f"- disk_type must be one of ['Standard HDD', 'Standard SSD', 'Premium SSD'].\n"
            f"- disk_size must be in the format '<number>GB' (e.g., '400GB').\n"
            f"- region must be a valid Azure region (e.g., 'US East').\n\n"
            f"Quote:\n{text}"
        )
...
# Function to fetch predefined choices for dropdowns
def get_predefined_choices():
    """
    Returns predefined choices for VM Sizes, Disk Types, and Regions.
    
    In a real-world scenario, these could be fetched dynamically from Azure APIs or stored in a configuration file.
    """
    vm_sizes = [
        'b2ms', 'b4ms', 'b8ms', 'b16ms', 'b32ms',
        'd2s_v3', 'd4s_v3', 'd8s_v3', 'd16s_v3', 'd32s_v3',
    ...
    regions = [
        'US East', 'US West', 'Europe West', 'South Central US',
        'East US 2', 'West Europe', 'Australia East', 'Japan East',
        # Add more regions as needed
    ]
    
    return vm_sizes, disk_types, regions
...
def main():
    ...
    # Sidebar: Azure Pricing Lookup with Dropdowns
    st.sidebar.header("Azure Pricing Lookup")
    
    # Predefined choices
    vm_sizes, disk_types, regions = get_predefined_choices()
    
    # Dropdowns and Autocomplete Inputs
    service_name = st.sidebar.selectbox("Select Azure Service Name", ["Virtual Machines"])  # Assuming 'Virtual Machines' is the primary service
    vm_size = st.sidebar.selectbox("Select VM Size", vm_sizes)
    disk_type = st.sidebar.selectbox("Select Disk Type", disk_types)
    
    # Disk Size Input with Validation
    disk_size = st.sidebar.text_input("Enter Disk Size (e.g., '400GB')", value="400GB")
    if disk_size:
        if not re.match(r'^\d+(\.\d+)?GB$', disk_size.strip(), re.I):
            st.sidebar.error("Disk size must be in the format '<number>GB' (e.g., '400GB').")
    
    region = st.sidebar.selectbox("Select Azure Region", regions)
    
    if st.sidebar.button("Fetch Pricing"):
        # Input Validation
        if not service_name:
            st.sidebar.error("Please select an Azure service.")
        elif not vm_size:
            st.sidebar.error("Please select a VM size.")
        elif not disk_type:
            st.sidebar.error("Please select a disk type.")
        elif not disk_size:
            st.sidebar.error("Please enter a disk size.")
        elif not region:
            st.sidebar.error("Please select a region.")
        elif not re.match(r'^\d+(\.\d+)?GB$', disk_size.strip(), re.I):
            st.sidebar.error("Disk size must be in the format '<number>GB' (e.g., '400GB').")
        else:
            with st.spinner("Fetching pricing data..."):
                # Log the pricing lookup
                logger.info(f"Pricing Lookup - Service: {service_name}, VM Size: {vm_size}, Disk Type: {disk_type}, Disk Size: {disk_size}, Region: {region}")
                ...
                if "error" in pricing_result:
                    ...
                    st.sidebar.subheader(f"Pricing for {vm_size} with {disk_size} {disk_type} in {region}")
                    ...
                    fig = px.bar(
                        ...
                        title=f"Pricing Breakdown for {vm_size} with {disk_size} {disk_type} in {region}"
                    )
                    fig.update_traces(texttemplate=f'${{y:.4f}}', textposition='outside')
                    fig.update_layout(uniformtext_minsize=8, uniformtext_mode='hide')
                    st.sidebar.plotly_chart(fig)
                    
                    logger.info(f"Pricing fetched successfully: {pricing_result}")
